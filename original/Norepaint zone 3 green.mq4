...
#property description "no repaint" // Note: This is optional now with the new parameter
...
extern string TimeFrame = 60; // TimeFrame
extern int HalfLength = 40; 
extern ENUM_APPLIED_PRICE Price = PRICE_CLOSE;
extern double BandsDeviations =1.8;
extern bool EnableRepaint = false; //Repaint
...
double tmBuffer[], upBuffer[], dnBuffer[], wuBuffer[], wdBuffer[], upArrow[], dnArrow[];

string IndicatorFileName;
bool calculatingTma = false, returningBars = false;
int timeFrame;
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
int init()
{
   ...
   IndicatorBuffers(7);
         ...
         SetIndexLabel (0, "TMA (" + IntegerToString(HalfLength) + ")");
         SetIndexBuffer(1,upBuffer); SetIndexDrawBegin(1,HalfLength);
         SetIndexBuffer(2,dnBuffer); SetIndexDrawBegin(2,HalfLength);
         SetIndexLabel (1, "TMA (" + IntegerToString(HalfLength) + ") upper band");
         SetIndexLabel (2, "TMA (" + IntegerToString(HalfLength) + ") lower band");
...
void calculateTma(int limit)
{
   ...
   for (i=limit; i>=0; i--)
   {
      ...
      for(j=1, k=HalfLength; j<=HalfLength; j++, k--)
      {
         ...
         sumw += k;
         if (EnableRepaint && j<=i) // Conditional: Include future terms only if repainting is enabled
         {
            sum += k*iMA(NULL,0,1,0,MODE_SMA,Price,i-j);
            sumw += k;
         }
